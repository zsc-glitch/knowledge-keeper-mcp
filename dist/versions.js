...
import { getVaultDir } from "./core.js";
// ============================================================
// Internal
// ============================================================
function getVersionsDir(vaultDir) {
    return path.join(vaultDir, ".versions");
}
async function ensureVersionsDir(vaultDir) {
    const dir = getVersionsDir(vaultDir);
    await fs.mkdir(dir, { recursive: true });
    return dir;
}
async function loadVersionHistory(vaultDir, id) {
    const dir = getVersionsDir(vaultDir);
...
async function saveVersionHistory(vaultDir, history) {
 ...
 */
export async function recordVersion(kp) {
 ...
 */
export async function diffVersions(id, versionA, versionB) {
    const vaultDir = getVaultDir();
    const history = await loadVersionHistory(vaultDir, id);
 ...
 */
export async function rollbackToVersion(id, targetVersion) {
    const vaultDir = getVaultDir();
    const history = await loadVersionHistory(vaultDir, id);
