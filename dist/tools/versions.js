 ...
 */
import { z } from "zod";
import { getVersionHistory, getVersion, diffVersions, rollbackToVersion, } from "../versions.js";
    ...
    server.registerTool("knowledge_versions", {
        ...
        try {
            ...
            if (action === "diff") {
                ...
                const diffLines = diffs.map(d => `📌 ${d.field}:\n  旧 (v${version}): ${d.old.substring(0, 200)}${d.old.length > 200 ? "..." : ""}\n  新 (v${compareWith}): ${d.new.substring(0, 200)}${d.new.length > 200 ? "..." : ""}`).join("\n\n");
                return {
