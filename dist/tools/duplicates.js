...
export function registerDuplicatesTool(server) {
    server.registerTool("knowledge_duplicates", {
        title: "重复检测",
        ...
        try {
            ...
            for (let i = 0; i < allEntries.length && groups.length < limit; i++) {
                if (visited.has(allEntries[i].id))
                    continue;
                const group = [allEntries[i]];
                for (let j = i + 1; j < allEntries.length; j++) {
                    if (visited.has(allEntries[j].id))
                        continue;
                ...
                if (group.length > 1) {
                    ...
                    const maxSim = group.slice(1).reduce((max, kp) => Math.max(max, computeSimilarity(group[0], kp, scope)), 0);
                    groups.push({
 ...
 */
function computeSimilarity(a, b, scope) {
    let titleSim = 0;
    let contentSim = 0;
