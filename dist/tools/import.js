...
export function registerImportTool(server) {
    server.registerTool("knowledge_import", {
        title: "导入知识",
 ...
 * 从 JSON 导入
 */
async function importFromJSON(content, overwrite) {
    const imported = [];
    let skipped = 0;
    let errors = 0;
    try {
 ...
 * 从 Markdown 导入
 */
async function importFromMarkdown(content, overwrite) {
    const imported = [];
    let skipped = 0;
    let errors = 0;
    // 按标题分割知识点
...
function validateType(type) {
    const validTypes = ["concept", "decision", "todo", "note", "project"];
