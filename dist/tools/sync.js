...
export function registerSyncTool(server) {
 ...
 * 从源拉取
 */
async function pullFromSource(sourcePath) {
    let imported = 0;
    let skipped = 0;
    let errors = 0;
    try {
...
function parseObsidianMarkdown(content) {
    ...
    // 类型推断
    let type = "note";
    if (body.includes("决定") || body.includes("决策"))
        type = "decision";
    if (body.includes("待办") || body.includes("TODO"))
        type = "todo";
    if (body.includes("项目") || body.includes("Project"))
        type = "project";
    if (body.includes("概念") || body.includes("定义"))
        type = "concept";
    return {
        type,
        title,
