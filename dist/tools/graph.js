...
export function registerGraphTool(server) {
 ...
 * 从根节点递归构建图谱
 */
async function buildGraphFromRoot(rootId, depth, nodes, edges, visited, currentDepth = 0) {
    if (currentDepth >= depth || visited.has(rootId))
        return;
    visited.add(rootId);
 ...
 * 构建全局图谱
 */
async function buildFullGraph(nodes, edges, minLinks) {
    // 需要从索引获取所有知识点
    // 这里简化处理，先获取标签相关的知识点
