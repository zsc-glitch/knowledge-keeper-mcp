...
async function loadManifest(vaultPath) {
...
async function saveManifest(vaultPath, manifest) {
    const path = join(vaultPath, MANIFEST_FILE);
...
// Cloud API Client
// ============================================================
async function apiCall(config, method, endpoint, body) {
 ...
 */
export async function pushChanges(config) {
 ...
 */
export async function pullChanges(config) {
    const manifest = await loadManifest(config.vaultPath);
    const result = await apiCall(config, "POST", "/api/sync/pull", {
        since: manifest.lastFullSync,
        deviceId: manifest.deviceId,
        knownVersions: Object.fromEntries(Object.entries(manifest.items).map(([id, rec]) => [id, rec.serverVersion])),
    });
    if (!result.success) {
 ...
 */
export async function getSyncStatus(vaultPath) {
